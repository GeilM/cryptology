...
unsigned SHA_1::circleShift(const unsigned& word, const int& bits){
    return (word << bits) | ((word) >> (32 - bits));
}

unsigned SHA_1::sha1Fun(const unsigned& B, const unsigned& C, const unsigned& D, const unsigned& t){
...
string SHA_1::sha1(const string& strRaw){

    string str(strRaw);

    str += (unsigned char)(0x80);

    long long unsigned len = str.size() << 3;
    while ((len) % 512 != 448){
        str += (char)0;
